...
Tarix:             ${date}
Saat:              ${time}
${d.isTakeaway ? "Sifariş" : "Masa"}:           ${d.tableName}
  ...
  return (
    <div className="flex flex-col md:flex-row gap-5 h-full">
      ...
      <div className="flex-1 min-w-0 flex flex-col">
            ...
            <button key={c.id}
              ...
              className={`px-4 py-2 rounded-full text-sm font-bold border transition-all cursor-pointer
                ${catId === c.id
                  ...
                  : "bg-white border-gray-200 text-gray-500 hover:border-[#FABB18] hover:text-[#FABB18]"}`}
        ...
        {/* Items grid */}
        <div className="grid grid-cols-2 sm:grid-cols-3 lg:grid-cols-4 gap-3 overflow-y-auto pb-2 pt-1">
              ...
              <button key={item.id}
                ...
                className={`bg-white border rounded-2xl p-4 text-left transition-all group
                  ${outOfStock
                    ...
                    : "border-gray-200 hover:border-[#FABB18] hover:-translate-y-0.5 hover:shadow-md cursor-pointer"}`}>
                ...
                <div className="text-sm font-extrabold text-[#3F2218] tabular-nums">
              ...
              return (
                <div key={li.itemId} className="flex items-center gap-2 py-3 border-b border-[#F1EEFA] last:border-0">
                  {item.imageUrl ? (
                    ...
                    <div className="w-9 h-9 rounded-lg bg-gray-100 flex items-center justify-center text-lg shrink-0">{getCatEmoji(data.categories.find(c => c.id === item.categoryId)?.name ?? "")}</div>
                  )}
        ...
        <div className="border-t border-gray-100 p-4 space-y-3">
          ...
          <div className="flex gap-2">
            <button onClick={cancelOrder}
              className="flex-1 py-2 rounded-xl bg-red-500 text-white text-sm font-bold hover:opacity-90 transition cursor-pointer">
              Ləğv et
            </button>
        ...
        <Modal onClose={() => setReceiptModal(false)} width="w-[340px]">
          ...
          <div className="flex gap-2 mt-4">
            ...
            <button onClick={printReceipt}
              ...
              style={{ background: "linear-gradient(135deg,#FABB18,#D4A017)" }}>🖨️ Çap et</button>
          </div>
        </Modal>
      )}
    </div>
