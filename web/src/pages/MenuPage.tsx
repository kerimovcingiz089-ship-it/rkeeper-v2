                ...
                {/* Image area */}
                  <div className="relative h-44 flex items-center justify-center overflow-hidden"
                    ...
                    {p.imageUrl ? (
                      ...
                      <span className="text-5xl group-hover:scale-110 transition-transform duration-300">{catName ? getCatEmoji(catName) : "🍽️"}</span>
                    )}
                  {catName && (
              ...
              {cart.map(c => (
                <div key={c.product.id} className="flex items-center gap-4 bg-rose-50/50 rounded-2xl p-4">
                  <div className="w-14 h-14 rounded-2xl flex items-center justify-center text-2xl shrink-0 overflow-hidden"
                    ...
                    {c.product.imageUrl ? (
                      <img src={c.product.imageUrl} alt={c.product.name} className="w-full h-full object-contain" />
                    ) : productEmoji(c.product)}
                  </div>
