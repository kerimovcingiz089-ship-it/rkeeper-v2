        ...
        <Modal onClose={() => setModal(null)}>
          ...
          <div className="relative">
            <input autoFocus value={catName} onChange={e => setCatName(e.target.value)}
              ...
              className="w-full px-3 py-2.5 pl-10 border border-gray-200 rounded-xl text-sm focus:outline-none focus:border-[#FABB18]" />
            <span className="absolute left-3 top-1/2 -translate-y-1/2 text-lg">{catName.trim() ? getCatEmoji(catName) : "📁"}</span>
          </div>
          <div className="flex gap-2 mt-5">
            ...
            <button onClick={saveCat} className="flex-1 py-2.5 rounded-xl text-white text-sm font-bold cursor-pointer" style={{ background: "linear-gradient(135deg,#FABB18,#D4A017)" }}>
          ...
          <div className="flex items-center gap-3 mb-3">
            {itemImage ? (
              ...
              <button onClick={() => fileRef.current?.click()}
                className="w-20 h-20 rounded-xl border-2 border-dashed border-gray-200 flex flex-col items-center justify-center text-gray-400 hover:border-[#FABB18] hover:text-[#FABB18] transition cursor-pointer">
