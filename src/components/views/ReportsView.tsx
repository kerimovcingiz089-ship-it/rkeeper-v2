    ...
    <div>
      ...
      <div className="grid grid-cols-2 sm:grid-cols-3 lg:grid-cols-5 gap-4 mb-5">
        ...
        <div className="bg-white border border-gray-200 rounded-2xl p-5">
          <div className="flex items-center gap-2 mb-2">
            <span className="w-6 h-6 rounded-lg bg-[#FEF3C7] flex items-center justify-center text-[11px]">🍽</span>
      ...
      <div className="flex gap-3 items-center mb-5 flex-wrap">
        <input type="date" value={reportDate} onChange={e => setReportDate(e.target.value)}
          className="px-3 py-2 border border-gray-200 rounded-full text-sm focus:outline-none focus:border-[#FABB18] cursor-pointer" />
                    ...
                    <tr key={h.id} className={`border-b border-gray-50 last:border-0 transition-colors ${h.isTakeaway ? 'bg-orange-50/40 hover:bg-orange-50/70' : 'hover:bg-gray-50/50'}`}>
                      ...
                      <td className="px-4 py-3 text-sm font-bold">
                        <div className="flex items-center gap-2">
                          <span className={`w-6 h-6 rounded-lg flex items-center justify-center text-[11px] flex-shrink-0 ${h.isTakeaway ? 'bg-orange-100' : 'bg-[#FEF3C7]'}`}>
                            {h.isTakeaway ? '📦' : '🍽'}
                          </span>
              ...
              <tbody>
                {sortedItemSales.length === 0 ? (
                  ...
                  <tr key={item.name} className="border-b border-gray-50 last:border-0">
                    ...
                    <td className="px-4 py-3 text-sm font-bold text-[#3F2218]">{item.qty} ədəd</td>
