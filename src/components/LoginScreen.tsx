...
export default function LoginScreen() {
  ...
  const inputStyle = (field: string) => ({
    ...
    border: `1.5px solid ${focused === field ? "#D4A017" : "rgba(255,255,255,.12)"}`,
    ...
    <div className="fixed inset-0 flex z-[500] overflow-hidden">
      ...
      <div className="hidden lg:flex relative flex-col items-center justify-center w-[52%] overflow-hidden"
        ...
        <div className="absolute w-[500px] h-[500px] rounded-full -top-32 -left-32 opacity-50 animate-[floatBlob_11s_ease-in-out_infinite]"
          style={{ background: "#FABB18", filter: "blur(90px)" }} />
        <div className="absolute w-[420px] h-[420px] rounded-full -bottom-40 -right-24 opacity-45 animate-[floatBlob_11s_ease-in-out_infinite_-3.5s]"
          style={{ background: "#D4A017", filter: "blur(80px)" }} />
        ...
        <div className="relative z-10 text-center text-white px-10">
          {/* Logo */}
          <div className="w-24 h-24 rounded-3xl mx-auto mb-8 flex items-center justify-center overflow-hidden"
            style={{
              background: "linear-gradient(135deg,#FABB18,#D4A017)",
              boxShadow: "0 20px 50px rgba(250,187,24,.5)",
            }}>
            <img src={logo} alt="Logo" className="w-full h-full object-contain" />
          </div>

          <h1 className="text-4xl font-extrabold tracking-tight mb-3">{data.settings.name}</h1>
          <p className="text-white/50 text-base font-medium">{t("login.tagline")}</p>

          {/* Decorative dots */}
          <div className="flex items-center justify-center gap-2 mt-10">
            <div className="w-2 h-2 rounded-full bg-[#FABB18] opacity-60" />
            <div className="w-2 h-2 rounded-full bg-[#D4A017] opacity-60" />
      ...
      <div className="flex-1 flex items-center justify-center relative overflow-hidden"
        ...
        <div className="absolute w-[350px] h-[350px] rounded-full -top-20 -right-20 opacity-30 animate-[floatBlob_13s_ease-in-out_infinite]"
          style={{ background: "#FABB18", filter: "blur(80px)" }} />
        <div className="absolute w-[300px] h-[300px] rounded-full -bottom-28 -left-16 opacity-25 animate-[floatBlob_13s_ease-in-out_infinite_-5s]"
          style={{ background: "#D4A017", filter: "blur(70px)" }} />
        ...
        <div className={`relative z-10 w-[400px] max-w-[88vw] rounded-3xl p-9 text-white transition-all
          ...
          <div className="hidden lg:block mb-8">
            <h2 className="text-2xl font-extrabold tracking-tight mb-1">{t("login.welcome")}</h2>
            <p className="text-white/45 text-sm">{t("login.subtitle")}</p>
          </div>

          {/* Mobile heading */}
          <div className="lg:hidden text-center mb-7">
            <h2 className="text-xl font-extrabold tracking-tight mb-1">{t("login.welcome")}</h2>
            <p className="text-white/45 text-sm">{t("login.subtitle")}</p>
          </div>
          ...
          <form onSubmit={handleSubmit} className="space-y-4">
            {/* Username */}
            <div>
              <label className="block text-[11px] font-bold text-white/40 uppercase tracking-wider mb-2 ml-1">{t("login.username")}</label>
              <div className="relative">
                <span className="absolute left-4 top-1/2 -translate-y-1/2 text-sm text-[#FABB18]">
            ...
            {/* Password */}
            <div>
              <label className="block text-[11px] font-bold text-white/40 uppercase tracking-wider mb-2 ml-1">{t("login.password")}</label>
              <div className="relative">
                <span className="absolute left-4 top-1/2 -translate-y-1/2 text-sm text-[#FABB18]">
            ...
            <button type="submit" disabled={loading || !username || !password}
              ...
              style={{
                background: "linear-gradient(135deg,#FABB18,#D4A017)",
                boxShadow: "0 6px 24px rgba(250,187,24,.4)",
              }}>
              {loading ? (
