          ...
          {data.categories.map(c => (
            <button key={c.id} onClick={() => setTakeawayActiveCat(c.id)}
              className={`px-4 py-2 rounded-full text-sm font-bold border transition-all cursor-pointer
                ${catId === c.id ? "text-white border-transparent" : "bg-white border-gray-200 text-gray-500 hover:border-[#FABB18] hover:text-[#FABB18]"}`}
              style={catId === c.id ? { background: "linear-gradient(135deg,#FABB18,#D4A017)" } : {}}>
              {c.name}
            </button>
          ))}
        </div>
            ...
            return (
              <button key={item.id} onClick={() => addToCart(item.id)} disabled={outOfStock}
                className={`bg-white border rounded-2xl p-4 text-left transition-all group
                  ${outOfStock ? "border-dashed border-gray-200 opacity-50 cursor-not-allowed" : "border-gray-200 hover:border-[#FABB18] hover:-translate-y-0.5 hover:shadow-md cursor-pointer"}`}>
                <div className="w-full h-28 flex items-center justify-center rounded-xl mb-2 bg-gray-50">
                  {item.imageUrl ? (
                    <img src={item.imageUrl} alt={item.name} className="w-full h-full object-contain" />
                  ) : (
                    <span className="text-5xl">{getCatEmoji(data.categories.find(c => c.id === item.categoryId)?.name ?? "")}</span>
                  )}
                </div>
                ...
                <div className="text-sm font-extrabold text-[#3F2218] tabular-nums">{fmtMoney(item.price, data.settings.currency)}</div>
              </button>
            );
          })}
        </div>
              ...
              return (
                <div key={o.id} className="bg-white border border-gray-200 rounded-2xl p-4 mb-3 flex items-center justify-between gap-3 hover:-translate-y-0.5 hover:shadow-md transition-all">
                  ...
                  <div className="flex items-center gap-2 flex-wrap justify-end">
                    ...
                    <span className="font-extrabold text-sm text-[#3F2218] tabular-nums">{fmtMoney(o.total, data.settings.currency)}</span>
                    <div className="flex gap-1">
                      ...
                      <button onClick={() => setData(prev => ({ ...prev, takeawayOrders: prev.takeawayOrders.map(x => x.id === o.id ? { ...x, status: "completed" } : x) }))}
                        ...
                        style={{ background: "linear-gradient(135deg,#FABB18,#D4A017)" }}>Təhvil</button>
      ...
      <div className="lg:w-[370px] flex-shrink-0 bg-white border border-gray-200 rounded-2xl flex flex-col shadow-sm overflow-hidden">
        ...
        <div className="px-4 pt-3">
          <input type="text" value={takeawayCustomerName} onChange={e => setTakeawayCustomerName(e.target.value)}
            ...
            className="w-full px-3 py-2.5 border border-gray-200 rounded-xl text-sm focus:outline-none focus:border-[#FABB18] mb-2" />
        </div>
            ...
            {(["cash","card"] as const).map(p => (
              <button key={p} onClick={() => setSelectedPay(p)}
                className={`flex-1 py-4 rounded-xl border-2 text-sm font-bold transition-all cursor-pointer
                  ${selectedPay === p ? "border-[#FABB18] bg-[#FEF3C7] text-[#3F2218]" : "border-gray-200 text-gray-400"}`}>
                {p === "cash" ? "💵 Nağd" : "💳 Kart"}
              </button>
            ))}
          </div>
      ...
      {receiptModal && receiptData && (
        <Modal onClose={() => setReceiptModal(false)} width="w-[340px]">
          ...
          <div className="flex gap-2 mt-4">
            ...
            <button onClick={printReceipt} className="flex-1 py-2.5 rounded-xl text-white text-sm font-bold cursor-pointer" style={{ background: "linear-gradient(135deg,#FABB18,#D4A017)" }}>🖨️ Çap et</button>
          </div>
        </Modal>
      )}
    </div>
