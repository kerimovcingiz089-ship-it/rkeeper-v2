    ...
    <footer className="relative overflow-hidden" style={{ background: "linear-gradient(180deg,#1a1a1a 0%,#111 100%)" }}>
      ...
      <div className="max-w-7xl mx-auto px-5 py-16">
        <div className="grid grid-cols-1 md:grid-cols-12 gap-12">
          ...
          {/* Newsletter */}
          <div className="md:col-span-4">
            <h4 className="font-display font-bold text-white text-sm mb-4">Yeniliklərdən xəbərdar olun</h4>
            <p className="text-gray-400 text-xs leading-relaxed mb-4">
              Endirimlər və yeni məhsullar haqqında ilk siz xəbər tutun
            </p>
            {subscribed ? (
              <div className="flex items-center gap-2 px-4 py-3.5 rounded-2xl bg-rose-500/10 border border-rose-500/20">
                <span className="text-sm">✅</span>
                <span className="text-xs text-rose-300 font-medium">Abunə olduğunuz üçün təşəkkürlər!</span>
              </div>
            ) : (
              <form onSubmit={handleSubscribe} className="flex gap-2">
                <input type="email" required value={email} onChange={e => setEmail(e.target.value)} placeholder="Email adresiniz"
                  className="flex-1 px-4 py-3 rounded-2xl bg-white/5 border border-white/10 text-white text-sm placeholder:text-gray-500 focus:outline-none focus:border-rose-400/50 transition" />
                <button type="submit"
                  className="px-5 py-3 rounded-2xl font-bold text-xs text-white transition hover:opacity-90 shrink-0"
          ...
          <div className="md:col-span-2">
            ...
            <div className="space-y-3 text-sm text-gray-400">
              <div className="flex items-center gap-3">
                <span className="w-8 h-8 rounded-lg bg-white/5 flex items-center justify-center text-xs shrink-0">📍</span>
